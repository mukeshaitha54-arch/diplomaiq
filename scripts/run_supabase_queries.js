...
async function run() {
  try {
    ...
    const { count: c1, error: e1 } = await supabase.from('academic_results').select('*', { count: 'exact', head: true });
    console.log('COUNT(*) FROM academic_results:', c1, e1?.message || '');

    // 2. SELECT COUNT(*) FROM semester_results;
    const { count: c2, error: e2 } = await supabase.from('semester_results').select('*', { count: 'exact', head: true });
    console.log('COUNT(*) FROM semester_results:', c2, e2?.message || '');

    // 3. SELECT COUNT(*) FROM attendance_records;
    const { count: c3, error: e3 } = await supabase.from('attendance_records').select('*', { count: 'exact', head: true });
    ...
    const { data: d4, error: e4 } = await supabase.from('semester_results').select('semester, sgpa, cgpa').order('semester');
    console.log('semester_results ORDER BY semester:');
    console.table(d4);

    // 5. SELECT DISTINCT semester FROM academic_results ORDER BY semester;
    // We will just select semester and distinct in memory
    const { data: d5, error: e5 } = await supabase.from('academic_results').select('semester');
    if (d5) {
